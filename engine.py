...
def train_step(model: torch.nn.Module,
    ...
    for batch, (X, y) in enumerate(tqdm(dataloader, desc="train batches", position=1, leave=False)):
...
def test_step(model: torch.nn.Module,
    ...
    with torch.inference_mode():
        ...
        for batch, (X, y) in enumerate(tqdm(dataloader, desc="test batches", position=1, leave=False)):
            X, y = X.to(device), y.to(device)  # Send data to target device
            test_pred_logits = model(X)  # Forward pass
            loss = loss_fn(test_pred_logits, y)  # Calculate loss
            test_loss += loss.item()  # Accumulate loss
            test_pred_labels = test_pred_logits.argmax(dim=1)  # Calculate accuracy
